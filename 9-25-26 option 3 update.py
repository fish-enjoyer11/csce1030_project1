...
while application_isactive == True: 
    ...
    if(user_input == 0):              #this is our input check. start by checking if its the exit command.
        ...
        if(user_input == 2): #BEGIN OPTION 2: Compute GPAs & Academic Averages - Matthew M
            ...
            if student_grades: #by using student_grades as the if-check, it returns True if the dict contains items. If its empty, returns False
                ...
                for stud_id in student_grades:
                    ...
                    print(f"\nID: {stud_id} | Avg Score: {gpa_avg:.1f}% | Letter: {student_lettergrades[stud_id]} | GPA: {student_gpas[stud_id]}")
        ...
        if(user_input == 3): #BEGIN OPTION 3: Display Formatted Grade  - Uriel Meneses
        


            Border = input("Enter a border character: ")

            if Border == "":
                Border = "*"
            print(f"\n{Border*20}\n  Formatted Grades \n{Border*20}") #i altered this line a little bit to make it a bit more stylish looking -Matthew

            for index, stud_id in enumerate(student_ids): #enumerate() returns a tuple of (index, value) for each item in the list

                tmp_standing = ""
                
                if student_gpas[stud_id] >= 3.5:
                    tmp_standing = "Dean's List"
                elif 2.0 <= student_gpas[stud_id] <= 3.4:
                    tmp_standing = "Good Standing"
                elif student_gpas[stud_id] < 2.0:
                    tmp_standing = "Probation Risk"

                student_tuple = tuple([stud_id, student_names[index], tmp_standing]) #create a tuple of the student info to print
                print(f"ID: {student_tuple[0]:<10} | Name: {student_tuple[1]:<20} | Standing: {student_tuple[2]}")
